...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    KF = KalmanFilter(0.2, 0.001, 3)
    # Kalman의 예측을 빠르게 따라가는 칼만필터

    KF2 = KalmanFilter(0.1, 1, 0.01)
    # YOLO를 빠르게 따라가는 칼만필터
    ...
    """
            ...
            param x_std_meas: x - 방향 측정의 표준 편차
            param y_std_meas: y - 방향 측정의 표준 편차
    """
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'{frame}')  # im.txt
